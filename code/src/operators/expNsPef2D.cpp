...
void expNsPef2D_v0::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    ...
    int nz_p_min, nz_p_max, nx_p_max;  
    ...
    for (int ix_r=nx_p - 1 ; ix_r<nx_d; ix_r++){
        ...
        for (int iz_r=nz_p - _lead - 1; iz_r<nz_d - _lead; iz_r++){
            ...
            nx_p_max=std::min(nx_p, ix_r/2 + 1);
            nz_p_max=std::min(nz_p, 1 + (iz_r+2*(int)_lead)/2);
            nz_p_min=std::max(0, (iz_r+2*(int)_lead-nz_d+2)/2);
            for (int ix_p=0; ix_p<nx_p_max; ix_p++){
                for (int iz_p=nz_p_min; iz_p<nz_p_max; iz_p++){
                    (*dat->_mat)[ix_r][iz_r] += (*pef->_mat)[ix_p][iz_p] * (*mod->_mat)[ix_r - 2*ix_p][iz_r - 2*iz_p + 2*_lead];
                }
            }
...
void expNsPef2D_v0::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
    ...
    for (int ix_r=nx_p - 1 ; ix_r<nx_d; ix_r++){
        ...
        for (int iz_r=nz_p - _lead - 1; iz_r<nz_d - _lead; iz_r++){
            ...
            for (int ix_p=0; ix_p<nx_p_max; ix_p++){
                for (int iz_p=nz_p_min; iz_p<nz_p_max; iz_p++){
                    (*mod->_mat)[ix_r - 2*ix_p][iz_r - 2*iz_p + 2*_lead] += (*pef->_mat)[ix_p][iz_p] * (*dat->_mat)[ix_r][iz_r];
                }
            }
...
void expNsPef2D_v1::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v1::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v2::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v2::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v3::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v3::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v4::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v4::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v5::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v5::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
...
void expNsPef2D_v9::forward(bool add, const std::shared_ptr<float2DReg> mod, std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
                ...
                for (int ix_p=0; ix_p<nx_p_max; ix_p++){
                    for (int iz_p=nz_p_min; iz_p<nz_p_max; iz_p++){
                        (*dat->_mat)[ix_r][iz_r] += (*newpef->_mat)[ix_p][iz_p]* //current pef without interpolation
                                                    ...
                                                    (*mod->_mat)[ix_r-2*ix_p][iz_r-2*iz_p+2*_lead];
                    }
                }
...
void expNsPef2D_v9::adjoint(bool add, std::shared_ptr<float2DReg> mod, const std::shared_ptr<float2DReg> dat) const {
    this->checkDomainRange(mod,dat);
    ...
    for (unsigned int iblk=0; iblk<blocks.size(); iblk++){
        ...
        for (int ix_r=xstart+nx_p-1; ix_r<xend; ix_r++){
            for (int iz_r = zstart+nz_p-1-_lead; iz_r<zend-_lead; iz_r++){
                ...
                for (int ix_p=0; ix_p<nx_p_max; ix_p++){
                    for (int iz_p=nz_p_min; iz_p<nz_p_max; iz_p++){
                        (*mod->_mat)[ix_r-2*ix_p][iz_r-2*iz_p+2*_lead] += (*newpef->_mat)[ix_p][iz_p]* //current pef without interpolation
                                                    //(dx*dz*(*newpef->_mat)[ix_p][iz_p]+ // current pef
                                                    //(dx-dx*dz)*(*pef_array[array_size-2]->_mat)[ix_p][iz_p]+ // upper pef
                                                    //(dz-dx*dz)*(*pef_array[1]->_mat)[ix_p][iz_p]+ // left pef
                                                    //(1-dx)*(1-dz)*(*pef_array[0]->_mat)[ix_p][iz_p]) * //upper left pef
