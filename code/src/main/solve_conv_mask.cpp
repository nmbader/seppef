...
// M is a mask operator (diagonal with 0 and 1)
// m0 = input data


int main(int argc, char **argv) {
    
    std::clog << "Starting program: solving min|| F.M.m - d ||_2 ; F = convolution operator, M is a mask \n";
	srand ( time(0) ); 
...
// read in the mask
    ...
    if (mask_file == "none") throw std::logic_error("The required mask is not provided.\n");
    std::shared_ptr<float2DReg> mask = VECEXT::sepRead(mask_file);
...
// set the data vector
    ...
    selection2D selection(mask); // define the masking operator
    chainOper2D chain(&conv, &selection);
