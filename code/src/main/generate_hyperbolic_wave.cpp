...
int main(int argc, char **argv) {
    ...
    std::clog << "Starting program generate_hyperbolic_wave \n";
...
// read in the parameters
    int nx, nz, nfx, nfz;
    float ox, oz, dx, dz, za, xa, vel, amp;
    bool interp;
    ...
    MB::readParam(argc, argv, "zapex", za, 0);
    MB::readParam(argc, argv, "xapex", xa, 0);
...
// filling data with a hyperbolic wave, subject to sinc interpolation if interp=true
    ...
    if ((interp==true) && (interp_type=="box")){
        for (int ix0=0; ix0<nx; ix0++){
            z = za*za + ((ix0*dx)-xa)*((ix0*dx)-xa)*(s*s);
            z = std::sqrt(z);
            iz0 = z/dz;
...
// write the data to disk
    ...
    std::clog << "Ending program generate_hyperbolic_wave; execution time (sec): " <<  duration.count()/1000000.0 << "\n";
    return 0;
}