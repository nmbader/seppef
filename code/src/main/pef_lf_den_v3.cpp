...
// solving min ||P'm|| by CGLS
// P' is a filter correlation matrix (adjoint of the convolution)
// m0 = input data

int main(int argc, char **argv) {
    
    std::clog << "Starting program pef_lf_denoise_v3 \n";
	srand ( time(0) ); 
...
// estimate the model (de-noised data)
    ...
    zero->zero();
    std::shared_ptr<hypercube> domain (new hypercube(Z, X));
    dat = VECEXT::pad(dat, VECEXT::getnx(pef) - 1, 0, VECEXT::getnz(pef) - 1, 0);
    VECEXT::copyHyper(dat, domain);
    
    convolution2D conv(pef,zero->getHyper(),dat->getHyper()); // define the PEF convolution matrix
	...
	cg.run(&adjConv, dat, zero);

    dat = VECEXT::cut(dat, VECEXT::getnx(pef) - 1, 0, VECEXT::getnz(pef) - 1, 0);
    VECEXT::copyHyper(dat, zero->getHyper());
...
// write the model to disk
    ...
    std::clog << "Ending program pef_lf_denoise_v3; execution time (sec): " <<  duration.count()/1000000.0 << "\n";
    return 0;
}