...
// D is a filter convolution matrix containing bank of filters
// L is an expanded filter convolution matrix containing bank of filters
// m0 = input data
...
int main(int argc, char **argv) {
    ...
    std::clog << "D = convolution with bank of filters, L = expanded convolution with bank of filters \n";
	srand ( time(0) ); 
...
// set the data vector
    ...
    varConv2D D(filb, mod->getHyper(), mod->getHyper(),
    nxb, nzb, xincb, zincb, leadb); // define the filter convolution operator D
    expVarConv2D L(fil, mod->getHyper(), dat->getHyper(),
