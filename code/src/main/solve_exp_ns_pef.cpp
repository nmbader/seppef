...
// solving min ||Ns_exp.m - d||
// Ns_exp is a non-stationary expanded PEF
// m0 = input data


int main(int argc, char **argv) {
    
    std::clog << "Starting program: solving min|| Ns_exp.m - d ||_2 ; Ns_exp = non-stationary expanded PEF \n";
	srand ( time(0) ); 
...
//set the operator
    ...
    switch (version){
		case 0:
        {
			std::shared_ptr<expNsPef2D_v0> nsP( new expNsPef2D_v0(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon));
			Ns = nsP;
        }
			break;
		...
		case 1:
        {
			std::shared_ptr<expNsPef2D_v1> nsP( new expNsPef2D_v1(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon));
            Ns = nsP;
        }
			break;
		...
		case 2:
        {
			std::shared_ptr<expNsPef2D_v2> nsP( new expNsPef2D_v2(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon));
        ...
        case 3:
        {
			std::shared_ptr<expNsPef2D_v3> nsP( new expNsPef2D_v3(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon, weightx));
            Ns = nsP;
        }
			break;
        ...
        case 4:
        {
			std::shared_ptr<expNsPef2D_v4> nsP( new expNsPef2D_v4(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon));
            Ns = nsP;
        }
			break;
        ...
        case 5:
        {
			std::shared_ptr<expNsPef2D_v5> nsP( new expNsPef2D_v5(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, epsilon));
			nsP->setWeightx(weightx);
            Ns = nsP;
        }
			break;
        ...
        case 9:
        {
			std::shared_ptr<expNsPef2D_v9> nsP( new expNsPef2D_v9(pef, dat0, mod->getHyper(), dat->getHyper(), pef_lead, winx, winz, pef_niter));
			Ns = nsP;
        }
			break;
