...
std::shared_ptr<complexDouble4DReg> complexDouble4DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complexDouble4DReg> x(new complexDouble4DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complexDouble4DReg> x(new complexDouble4DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complexDouble4DReg> complexDouble4DReg::cloneSpace() const {
  std::shared_ptr<complexDouble4DReg> x(new complexDouble4DReg(getHyper()));
  x->_mat = 0;
...
void complexDouble4DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new complexDouble4D(
      boost::extents[axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complexDouble4DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complexDouble4D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<complexDouble4DReg> complexDouble4DReg::window(
  ...
  std::shared_ptr<complexDouble4DReg> out(new complexDouble4DReg(hypOut));
