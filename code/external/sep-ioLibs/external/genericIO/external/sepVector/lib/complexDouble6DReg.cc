...
std::shared_ptr<complexDouble6DReg> complexDouble6DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complexDouble6DReg> x(new complexDouble6DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complexDouble6DReg> x(new complexDouble6DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complexDouble6DReg> complexDouble6DReg::cloneSpace() const {
  std::shared_ptr<complexDouble6DReg> x(new complexDouble6DReg(getHyper()));
  x->_mat = 0;
...
void complexDouble6DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new complexDouble6D(boost::extents[axes[5].n][axes[4].n][axes[3].n]
                                         [axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complexDouble6DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complexDouble6D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<complexDouble6DReg> complexDouble6DReg::window(
  ...
  std::shared_ptr<complexDouble6DReg> out(new complexDouble6DReg(hypOut));
