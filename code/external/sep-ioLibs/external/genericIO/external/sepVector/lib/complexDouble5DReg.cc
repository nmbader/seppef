...
std::shared_ptr<complexDouble5DReg> complexDouble5DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complexDouble5DReg> x(new complexDouble5DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complexDouble5DReg> x(new complexDouble5DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complexDouble5DReg> complexDouble5DReg::cloneSpace() const {
  std::shared_ptr<complexDouble5DReg> x(new complexDouble5DReg(getHyper()));
  x->_mat = 0;
...
void complexDouble5DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new complexDouble5D(
      boost::extents[axes[4].n][axes[3].n][axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complexDouble5DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complexDouble5D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<complexDouble5DReg> complexDouble5DReg::window(
  ...
  std::shared_ptr<complexDouble5DReg> out(new complexDouble5DReg(hypOut));
