...
std::shared_ptr<complexDouble3DReg> complexDouble3DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complexDouble3DReg> x(new complexDouble3DReg(getHyper()));
    return x;
  } else {
    std::shared_ptr<complexDouble3DReg> x(new complexDouble3DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complexDouble3DReg> complexDouble3DReg::cloneSpace() const {
  std::shared_ptr<complexDouble3DReg> x(new complexDouble3DReg(getHyper()));
  x->_mat = 0;
  x->setSpace();
...
void complexDouble3DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  ...
  _mat.reset(new complexDouble3D(boost::extents[axes[2].n][axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complexDouble3DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complexDouble3D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
  if (axes.size() != 3)
    throw(SEPException(std::string("Axes size must be 3 is ") +
...
std::shared_ptr<complexDouble3DReg> complexDouble3DReg::window(
  ...
  std::shared_ptr<complexDouble3DReg> out(new complexDouble3DReg(hypOut));
