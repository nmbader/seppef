...
std::shared_ptr<complexDouble2DReg> complexDouble2DReg::clone() const {
  if (getSpaceOnly()) {
    std::shared_ptr<complexDouble2DReg> x(new complexDouble2DReg(getHyper()));

    return x;
  } else {
    std::shared_ptr<complexDouble2DReg> x(new complexDouble2DReg(getHyper(), *_mat));

    return x;
  }
}
std::shared_ptr<complexDouble2DReg> complexDouble2DReg::cloneSpace() const {
  std::shared_ptr<complexDouble2DReg> x(new complexDouble2DReg(getHyper()));
  x->_mat = 0;
  x->setSpace();
...
void complexDouble2DReg::initNoData(std::shared_ptr<SEP::hypercube> hyp) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
  if (2 != axes.size()) throw(SEPException("axes must of length 2"));

  _mat.reset(new complexDouble2D(boost::extents[axes[1].n][axes[0].n]));
  setData(_mat->data());
}
void complexDouble2DReg::initData(std::shared_ptr<SEP::hypercube> hyp,
                            const complexDouble2D &vals) {
  const std::vector<SEP::axis> axes = hyp->getAxes();
  setHyper(hyp);
...
std::shared_ptr<complexDouble2DReg> complexDouble2DReg::window(
  ...
  std::shared_ptr<complexDouble2DReg> out(new complexDouble2DReg(hypOut));
